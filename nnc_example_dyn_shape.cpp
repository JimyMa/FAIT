...
int main() {
  ...
  // Compute Op Define
  c_tensor = Compute("c_buf", {N, N}, [&](const std::vector<VarHandle>& axes) {
    return a_buf.load(axes[0], axes[1]) + b_buf.load(axes[0], axes[1]);
  });

  d_tensor = Compute("d_buf", {N, N}, [&](const std::vector<VarHandle>& axes) {
    return b_buf.load(axes[0], axes[1]) + c_tensor.load(axes[0], axes[1]);
  });

  e_tensor = Compute("e_buf", {N}, [&](const std::vector<VarHandle>& axes) {
  ...
  stmt_ = FunctorParallization::parallel_functor_shape(
      stmt_, 3, new_loop_axis.node(), {{N.node(), {N_0, N_1, N_2}}});
  ...
  auto a_1_runtime =
      at::ones({n_1_runtime, n_1_runtime}, at::kDouble).cuda() * 2.0;
  auto a_2_runtime =
      at::ones({n_2_runtime, n_2_runtime}, at::kDouble).cuda() * 3.0;
  ...
  std::vector<c10::IValue> inputs = {
      a_0_runtime, a_1_runtime,      a_2_runtime, b_0_runtime, b_1_runtime,
      b_2_runtime, scalar_0_runtime, n_0_runtime, n_1_runtime, n_2_runtime};
