...
class DeadCodeEliminator {
 ...
 private:
    ...
    for (Node* node : block->nodes()) {
      ...
      for (size_t i = 0; i < g.inputs().size(); ++i) {
        if (!g.inputs().at(i)->hasUses()) {
          GRAPH_UPDATE(
              "Dead ", i, "-th input ", node->inputs().at(i)->debugName(), "(",
              g.inputs().at(i)->debugName(), " in a subgraph) will be removed");
          g.eraseInput(i);
          node->removeInput(i);
        }
      }
  ...
  void sweep(Block* block, bool recurse) {
    ...
    for (auto it = nodes.begin(); it != nodes.end(); it++) {
      ...
      if (!(marked_.count(node) || node->hasUses())) {
        GRAPH_UPDATE(
            "Node ", it->kind().toQualString(), " which outputs ",
      ...
      if (!node->outputs().at(i)->hasUses()) {
        ...
        for (Block* b : node->blocks()) {
          GRAPH_UPDATE("\tCorresponding block output ",
                       b->outputs().at(i)->debugName(), " will be removed");
          b->eraseOutput(i);
        }
      }
    }
  ...
  void logDeadLoopOutputs(Node* node, size_t i, size_t loop_input_offset,
    ...
    GRAPH_UPDATE("Dead ", i, "-th output ", node->outputs().at(i)->debugName(),
                 " will be removed");
