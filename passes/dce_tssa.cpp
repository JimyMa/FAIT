...
public:
    ...
    // The algorithm is an inverse mark-and-sweep. Starting from the return
    // node, we mark "live" nodes that are necessary for the output. Nodes that
    // have side effects are also marked.
    void run(Block* block, bool recurse) {
        ...
        // Initialize by marking the return node and all its consumed values as
        // live
...
private:
    void eliminateDeadForkInputs(Block* block, bool recurse) {
        for (Node* node : block->nodes()) {
            ...
            // WARNING: Do not use a ranged loop. The loop bounds are changed by
            // the loop body.
            for (size_t i = 0; i < g.inputs().size(); ++i) {
                if (!g.inputs().at(i)->hasUses()) {
                    GRAPH_UPDATE("Dead ", i, "-th input ",
    ...
    bool markReturnNode(Node* node) {
        ...
        if (outerNode == nullptr || outerNode->kind() == prim::Reverse) {
            ...
            // return block. We consider all graph outputs to be "used", so just
            // mark this node normally.
            return mark(node);
        }
        ...
        if (outerNode->kind() == prim::Loop ||
            ...
            for (const auto i : c10::irange(loop.carriedOutputs().size())) {
                if (outerNode->kind() == c10::onnx::Loop) {
                    ...
                    // The number of body carried inputs and outputs are
                    // different. They cannot be mapped to each other easily by
                    // the same index.
            ...
            // Also mark the loop next condition as live, since it will be used
    ...
    // `b` are dead, even though `b[0] += 1` mutates a live memory location
    // (since `b[0]` is an alias of `a`). i.e. `a` is used to compute `tot` in
    // the next iteration
    //
    ...
    void sweep(Block* block, bool recurse) {
        ...
        for (auto it = nodes.begin(); it != nodes.end(); it++) {
            ...
            if (!(marked_.count(node) || node->hasUses())) {
                GRAPH_UPDATE("Node ", it->kind().toQualString(),
                             " which outputs ",
                             (node->outputs().size() > 0
                                  ? node->outputs().at(0)->debugName()
                                  : "n/a"),
                             " will be removed");
                it.destroyCurrent();
            }
    ...
    void removeDeadBlockOutputs(Node* node) {
        ...
        for (size_t i_1 = node->outputs().size(); i_1 > 0; --i_1) {
            ...
            if (!node->outputs().at(i)->hasUses()) {
                GRAPH_UPDATE("Dead ", i, "-th output ",
                             node->outputs().at(i)->debugName(), " of node ",
                             node->kind().toQualString(), " will be removed");
                node->eraseOutput(i);
                for (Block* b : node->blocks()) {
                    GRAPH_UPDATE("\tCorresponding block output ",
    ...
    void removeDeadLoopOutputs(Node* node) {
        if (node->kind() != prim::Loop) return;
        auto loop_body = node->blocks().at(0);
        auto loop_input_offset =
            2;                      // offset of loop carried deps in input list
        auto loop_body_offset = 1;  // offset to the loop carried dependencies
                                    // in block inputs/outputs

        for (size_t i_1 = node->outputs().size(); i_1 > 0; --i_1) {
            size_t i = i_1 - 1;
            if (!node->outputs().at(i)->hasUses() &&
                !loop_body->inputs().at(loop_body_offset + i)->hasUses()) {
                logDeadLoopOutputs(node, i, loop_input_offset,
                                   loop_body_offset);
                node->eraseOutput(i);
    ...
    void logDeadLoopOutputs(Node* node, size_t i, size_t loop_input_offset,
                            size_t loop_body_offset) {
        auto loop_body = node->blocks().at(0);
        GRAPH_UPDATE("Dead ", loop_input_offset + i, "-th input ",
                     node->inputs().at(i)->debugName(), " will be removed");
        GRAPH_UPDATE("Dead ", i, "-th output ",
                     node->outputs().at(i)->debugName(), " will be removed");
        GRAPH_UPDATE("\tDead block input ",
                     loop_body->inputs().at(loop_body_offset + i)->debugName(),
                     "at offset ", loop_body_offset + i, " will be removed");
        GRAPH_UPDATE("\tDead block output ",
