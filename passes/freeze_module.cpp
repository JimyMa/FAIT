      ...
      for (auto n : block->nodes()) {
        ...
        if (n->kind() == prim::SetAttr || n->kind() == prim::GetAttr) {
          ...
          applyToForkSubgraph(
              ...
              std::bind(&AttributePropagator::recordMutableAttrs, *this,
                        std::placeholders::_1));
        }
      }
    }
      ...
      for (auto n : block->nodes()) {
        ...
        if (n->kind() == prim::GetAttr) {
          ...
          applyToForkSubgraph(
              n, graph,
              // NOLINTNEXTLINE(modernize-avoid-bind)
              std::bind(&AttributePropagator::inlineInterfaceCalls, *this,
                        std::placeholders::_1, interfacesToRetype));
        }
      }
    }
  }
        ...
        if (n->kind() == prim::GetAttr) {
          ...
          if (!paramConst) {
            ...
            if (auto attrVal = tryInsertConstant(*graph, attr)) {
              ...
              GRAPH_DEBUG(attr.type()->cast<ClassType>() ? "" : "attribute: ",
          ...
          applyToForkSubgraph(
              n, graph,
              // NOLINTNEXTLINE(modernize-avoid-bind)
              std::bind(&AttributePropagator::propagateAttributes, *this,
                        std::placeholders::_1));
        }
      }
    }
      ...
      for (auto n : block->nodes()) {
        ...
        if (n->kind() == prim::GetAttr) {
          ...
          applyToForkSubgraph(
              n, graph,
              // NOLINTNEXTLINE(modernize-avoid-bind)
              std::bind(&AttributePropagator::recordReferencedAttrs, *this,
                        std::placeholders::_1));
        }
      }
    }
