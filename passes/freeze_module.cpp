...
public:
        ...
        auto checkName = [this](std::string& name) {
            ...
            if (resolved_name) {
                ...
                if (parent_module.hasattr(attr_name)) {
                    ...
                    // cleaning the frozen module, make sure it will be
                    // preserved entirely.
                    if (value.isModule()) {
                        preservedSubModule_.insert(value.toModule()._ivalue());
                    }
                    insertMutableAttr(attr_name, value,
                                      parent_module._ivalue());
                } else {
    ...
    void run() {
        ...
        for (auto function : preservedMethods_) {
            GRAPH_DEBUG("Recording mutable attrs for function: " +
                        function->name());
            auto graph = toGraphFunction(*function).graph();
...
private:
    ...
    c10::optional<ResolvedName> resolveName(const std::string& name) {
        ...
        for (size_t i = 0; i < sub_names.size() - 1; ++i) {
            ...
            for (const auto& child_module : cur_module.named_children()) {
                if (child_module.name == sub_name) {
                    attr_infos.emplace_back(cur_module._ivalue(),
                                            child_module.name);
        ...
        if (cur_module.hasattr(attr_name) ||
            cur_module.find_method(attr_name)) {
    ...
    // analysis.
    //
    // We can use a more efficient algorithm to hash each constant GetAttr to
    ...
    void recordMutableAttrs(std::shared_ptr<Graph>& graph) {
        ...
        while (!blocks.empty()) {
            ...
            for (auto n : block->nodes()) {
                ...
                // Modules with prim::ModuleContainerIndex cannot be frozen
                // because they return InterfaceTypes.
                TORCH_CHECK(n->kind() != prim::ModuleContainerIndex,
                            "Freezing modules containing "
                            "prim::ModuleContainerIndex is not supported");

                if (n->kind() == prim::SetAttr || n->kind() == prim::GetAttr) {
                    // By default if interface attributes are present then fail
                    // freezing. If freezingInterfaces is on then Interfaces are
                    // folded similarly to other attributes.
                    ...
                    if (n->kind() == prim::GetAttr) {
                        ...
                        // Do not record submodules. Their attributes are
                    ...
                    if (n->kind() == prim::SetAttr ||
                        aliasDb->hasOutputWriters(n)) {
                        GRAPH_DEBUG(n->kind() == prim::GetAttr
                                        ? "attribute: " + name + " in %" +
                                              n->output()->debugName() +
                                              " has inplace writer"
                    ...
                    applyToForkSubgraph(
                        ...
                        std::bind(&AttributePropagator::recordMutableAttrs,
        ...
        for (auto use : n->output()->uses()) {
            ...
            if (user_node->kind() == prim::CallMethod) {
                ...
                if (auto graphFunction = tryToGraphFunction(function)) {
                    GRAPH_UPDATE("Inlining interface method '", function.name(),
                                 "' to ", *user_node);

                    GRAPH_UPDATE("Function body: ",
                                 graphFunction->optimized_graph());
    ...
    // separate step from normal inlining because CallMethod on an interface
    // type requires extra steps compared to inlining a normal CallMethod.
    //
    // Next we need to simplify the structure of the module data structure,
    // which is done for the most part by the usual steps in
    // cleanupFrozenModule.
    //
    ...
    //   2. call reassignInterfaceTypes, which reassigns interface types to
    //   their
    ...
    //      separate step)
    //   3. eventually cleanupFrozenModule will reorder the module data
    //   structure
        ...
        while (!blocks.empty()) {
            ...
            for (auto n : block->nodes()) {
                ...
                if (n->kind() == prim::GetAttr) {
                    ...
                    // Check to make sure we're not assigning the value of any
                    // parameters that are interface types. See [Note: Inlining
                    // interfaces strategy]
                    auto name = n->s(attr::name);
                    auto attrModule = module_;
                    auto input = n->inputs()[0];
                    ...
                    // note: this will modify attrModule until it is the parent
                    // of the "name" attr. In other words, attrModule is now the
                    // module that matches "input". We can't use
                    ...
                    const auto& attrType =
                        attrModule.type()->getAttribute(name);
                    ...
                    applyToForkSubgraph(
                        n, graph,
                        // NOLINTNEXTLINE(modernize-avoid-bind)
                        std::bind(&AttributePropagator::inlineInterfaceCalls,
                                  *this, std::placeholders::_1,
                                  interfacesToRetype));
                }
            }
    ...
    void reassignInterfaceTypes(
        ...
        for (const auto& it : interfacesToRetype) {
            ...
            for (const std::string& name : it.second) {
                ...
                attrModule.type()->unsafeChangeAttributeType(name,
                                                             subvalueType);
            }
        }
    }
            ...
            for (auto it = block->nodes().begin();
                ...
                if (n->kind() == prim::GetAttr) {
                    ...
                    if (!findConstantAttr(input, name, attrModule, graph)) {
                        GRAPH_DEBUG(input->type()->cast<InterfaceType>() ||
                                            input->type()
                                                ->expectRef<ClassType>()
                                                .is_module()
                    ...
                    if (!paramConst) {
                        ...
                        if (auto attrVal = tryInsertConstant(*graph, attr)) {
                            ...
                            GRAPH_DEBUG(attr.type()->cast<ClassType>()
                                            ? ""
                                            : "attribute: ",
                                        name, " is not materializable.");
                            continue;
                        }
                    ...
                    GRAPH_UPDATE("Folding GetAttr %",
                                 n->outputs()[0]->debugName(), " with ",
                    ...
                    applyToForkSubgraph(
                        n, graph,
                        // NOLINTNEXTLINE(modernize-avoid-bind)
                        std::bind(&AttributePropagator::propagateAttributes,
                                  *this, std::placeholders::_1));
                }
            }
        }
    ...
    void applyToForkSubgraph(
        ...
        // as the base module (similar to 'self' in forward) to resolve
        // GetAttrs.
                ...
                if (n->kind() == prim::GetAttr) {
                    ...
                    for (auto& mptr : modules) {
                        ...
                        if (module.type() == n->inputs()[0]->type()) {
                            ...
                            // more clear, these are attributes we could not
                            // inline, which include other reasons besides
                            // mutation (unsupported constant, getAttr resolving
                            // to non-getAttr node, etc)
                            insertMutableAttr(name, attr, mptr);
                            if (attr.isModule()) {
                    ...
                    applyToForkSubgraph(
                        n, graph,
                        // NOLINTNEXTLINE(modernize-avoid-bind)
                        std::bind(&AttributePropagator::recordReferencedAttrs,
                                  *this, std::placeholders::_1));
                }
            }
        }
    ...
    void handleSharedClassType(Module& module, std::shared_ptr<Graph>& graph) {
        ...
        for (const auto i : c10::irange(N)) {
            ...
            if (isMutableType(attrTy)) {
                ...
                isMutable = it2 != preservedScalarAttrs_.end() &&
                            it2->second.count(name);
            }
            if (isMutable) {
    ...
    void removeUnusedAttrs() {
        ...
        for (auto& it : attrsToKeep_) {
            ...
            for (auto fn : funcsToRemove) {
                ...
                mod._ivalue()->compilation_unit()->unsafeRemoveMethod(
                    fn->qualname());
            }
    ...
    // Contains the attribute slots that need to be preserved for each
    // ClassType.
    ...
    std::unordered_map<c10::intrusive_ptr<at::ivalue::Object>,
