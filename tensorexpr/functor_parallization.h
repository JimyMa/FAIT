...
class TORCH_API ParallelFunctorInputArgsMutator : public IRMutator {
 public:
    ParallelFunctorInputArgsMutator(int64_t degree,  /* How many tensor in list? */
                             VarPtr& iter_var,
                             std::unordered_map<BufPtr, std::vector<BufHandle>>& buf_args_map,
                             std::unordered_map<VarPtr, std::vector<VarHandle>>& var_args_map /* new args */)
      :  degree_(degree),
         iter_var_(iter_var),
 ...
 private:
  ExprPtr expr_replace(LoadPtr functor_arg, std::vector<BufHandle>& parallel_args);
  int64_t degree_;
  VarPtr iter_var_;
...
public:
    ParallelFunctorOutputArgsMutator(int64_t degree,  /* How many tensor in list? */
                                    VarPtr& iter_var,
                                    std::unordered_map<BufPtr, std::vector<BufHandle>>& args_map  /* new args */)
            :  degree_(degree),
               iter_var_(iter_var),
               args_map_(args_map) {}
...
private:
    StmtPtr stmt_replace(StorePtr functor_arg, std::vector<BufHandle>& parallel_args);
    int64_t degree_;
    VarPtr iter_var_;
...
public:
    ParallelFunctorShapeDimsMutator(int64_t degree,  /* How many tensor in list? */
                                    VarPtr& iter_var,
                                    std::unordered_map<VarPtr, std::vector<VarHandle>>& args_map  /* new args */)
            :  degree_(degree),
               iter_var_(iter_var),
               args_map_(args_map) {}
...
private:
    ExprPtr expr_replace(VarPtr functor_arg, std::vector<VarHandle>& parallel_args);
    int64_t degree_;
    VarPtr iter_var_;
...
class TORCH_API FunctorParallization {
 public:
  static StmtPtr parallel_functor_load(StmtPtr s,  /* Functor Statement */
                                       int64_t degree,  /* How many tensor in list? */
                                       VarPtr iter_var,
                                       std::unordered_map<BufPtr, std::vector<BufHandle>> buf_args_map,
                                       std::unordered_map<VarPtr, std::vector<VarHandle>> var_args_map = {});

  static StmtPtr parallel_functor_store(StmtPtr s,
                                        int64_t degree,
                                        VarPtr iter_var,
                                        std::unordered_map<BufPtr, std::vector<BufHandle>> args_map);

  static StmtPtr parallel_functor_shape(StmtPtr s,
                                        int64_t degree,
                                        VarPtr iter_var,
