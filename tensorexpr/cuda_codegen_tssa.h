...
class CudaAnalysisTssa : public IRVisitor {
 public:
  CudaAnalysisTssa() {
    gpu_block_extents_ = {alloc<IntImm>(1), alloc<IntImm>(1), alloc<IntImm>(1)};
    gpu_thread_extents_ = {alloc<IntImm>(1), alloc<IntImm>(1),
                           alloc<IntImm>(1)};
  }
 ...
 private:
  void visit(StorePtr v) override { store_targets_.insert(v->buf()); }
...
class GPUMetaVarRewriterTssa : public IRMutator {
 public:
  // NOLINTNEXTLINE(cppcoreguidelines-pro-type-member-init)
  explicit GPUMetaVarRewriterTssa(const CudaAnalysisTssa* cuda_analysis)
      : cuda_analysis_(cuda_analysis) {
    gpu_block_vars_ = {alloc<Var>("blockIdx.x", kInt),
                       alloc<Var>("blockIdx.y", kInt),
                       alloc<Var>("blockIdx.z", kInt)};
    gpu_thread_vars_ = {alloc<Var>("threadIdx.x", kInt),
                        alloc<Var>("threadIdx.y", kInt),
                        alloc<Var>("threadIdx.z", kInt)};

    current_block_reach_ = {alloc<IntImm>(1), alloc<IntImm>(1),
                            alloc<IntImm>(1)};
    current_thread_reach_ = {alloc<IntImm>(1), alloc<IntImm>(1),
                             alloc<IntImm>(1)};
  }
  ...
  const std::vector<VarPtr>& gpu_block_vars() const { return gpu_block_vars_; }
 ...
 private:
  ...
  const CudaAnalysisTssa* cuda_analysis_;
};

// A class that overrides the underlying IRPrinter to produce Cuda C.
class CudaPrinterTssa : public IRPrinter {
 public:
  explicit CudaPrinterTssa(std::ostream* os,
                           const CudaAnalysisTssa* cuda_analysis,
                           bool has_random)
...
class TORCH_CUDA_CU_API CudaCodeGenTssa : public CodeGen {
 public:
  // template <typename... Ts>
  // // NOLINTNEXTLINE(cppcoreguidelines-pro-type-member-init)
  // CudaCodeGenTssa(StmtPtr stmt, Ts... ts)
  //     : CodeGen(stmt, std::vector<BufferArg>({BufferArg(ts)...}),
  //               at::Device(at::kCUDA, at::cuda::current_device())) {
  //   Initialize();
  // }

  // NOLINTNEXTLINE(cppcoreguidelines-pro-type-member-init)
  CudaCodeGenTssa(StmtPtr stmt, const std::vector<BufferArg>& buffer_args,
                  at::Device device = at::Device(at::kCUDA,
 ...
 private:
  ...
  std::unique_ptr<CudaPrinterTssa> printer_;
  std::unique_ptr<CudaAnalysisTssa> cuda_analysis_;
  std::unique_ptr<GPUMetaVarRewriterTssa> metavar_rewriter_;
