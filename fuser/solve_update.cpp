...
std::vector<Node*> BufferDisjointSet::get_pass_up_chain(Value* value) {
  ...
  if (!buffer_tree)
    std::cout << "cannot find buffer tree when solve update " << value->debugName() << std::endl;
...
void UpdateSolver::run() {
  ...
  for (auto node = block->nodes().front(); node != block->nodes().back();) {
    if (view_symbol.count(node->kind())) {
      ...
      std::vector<Node*> pass_up_chain = buffer_disjoint_set_->get_pass_up_chain(updator);
      std::vector<Node*> pass_down_chain = buffer_disjoint_set_->get_pass_down_chain(src);
