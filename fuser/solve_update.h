...
struct BufferNode {
 public:
  ...
  std::vector<BufferNodePtr>& views() {
    return views_;
  }

  void add_view(BufferNodePtr buffer_node) {
    views_.push_back(buffer_node);
  }
...
struct BufferTree {
 public:
  ...
  BufferNodePtr find_parent(BufferNodePtr child) {
    ...
    for (auto node : pre_order_list) {
      auto views = node->views();
      std::vector<BufferNodePtr>::iterator iter = find(views.begin(),views.end(), child);
      if (iter != views.end()) {
        return node;
      } 
    }
 ...
 public:
  ...
  BufferTreePtr find_buffer_tree(Value* value) {
    for (auto &root : roots_) {
      if (root->find_buffer_node(value))
        return root;
    }
    return nullptr;
  }
