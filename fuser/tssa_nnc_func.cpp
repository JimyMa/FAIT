...
Tensor computeSliceSet(
  ...
  return Compute(
          ...
          [&](const std::vector<VarHandle>& axes) {
            ...
            auto cond_0 = CompareSelect::make(
              axes[dim],
              end_expr,
              src->load(axes),
...
computeSliceShape(std::vector<ArgValue> input_args) {
  ...
  result[dim] = Min::make((end_expr - start_expr) / step_expr, src->dim(dim), true);
  return result;
}

std::vector<ExprHandle>
computePermuteShape(std::vector<ArgValue> input_args) {
  auto src = c10::get_if<BufHandle>(&input_args[0]);
